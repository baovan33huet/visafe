...
@section('content')
    ...
    <div class="border">
        ...
        <form class="shadow-sm p-3 bg-light rounded" method="post" action="{{route('admin.teacher.store')}}">
            @csrf
            <div class="form-row">
                {{-- NAME--}}
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                    ...
                    <input type="number" min="0" class="form-control   {{$errors->has('exp')? ' is-invalid':false }}" name="exp" id="inputEmail4" placeholder="2 years" value="{{old('exp')}}">
                    @error('exp')
                    <div class="invalid-feedback">
                        {{$message}}
                    </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-12">
                    ...
                    <textarea class="form-control ckeditor  {{$errors->has('description')? ' is-invalid':false}}" name="description" id="detail" style="min-height: 500px;" placeholder="" >{{old('description')}}</textarea>
                    @error('description')
                    <div class="invalid-feedback">
                        {{$message}}
                    </div>
            ...
            <div class="form-row {{ $errors->has('image') ? 'align-items-center' : 'align-items-end' }}">
                <div class="col-md-6 mb-2">
                    ...
                    <input type="text" class="form-control {{$errors->has('image') ? ' is-invalid':false}}" name="image" id="image" placeholder="" value="{{old('image')}}">
                    @error('image')
                    <div class="invalid-feedback">
                        {{$message}}
                    </div>
                ...
                <div class="col-md-4" id="holder">
                    @if ( old('image') )
                        <img src="{{old('image')}}" style="width: 80px; height: 80px;" alt="">
                    @endif
                </div>
